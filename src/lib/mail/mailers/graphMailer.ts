// src/lib/mail/mailers/graphMailer.ts
...
export async function sendMailGraphAppOnly(params: {
  ...
  const res = await fetch(
    `${GRAPH_BASE}/users/${encodeURIComponent(params.from)}/sendMail`,
    {
      method: "POST",
      headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`sendMailGraphAppOnly failed: ${res.status} ${raw}`);
  }
}
