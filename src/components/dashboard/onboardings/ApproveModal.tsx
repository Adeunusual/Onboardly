...
export function ApproveModal({
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      await onConfirm(v || undefined);
      onClose();
    } catch (e) {
      if (e instanceof ApiError) setError(e.message);
      else if (e instanceof Error) setError(e.message);
      else setError("Unable to approve right now. Please try again.");
    } finally {
      setSaving(false);
    }
  }
    ...
    >
      ...
      {error && (
        <div className="mt-4 rounded-xl border border-[var(--dash-red-soft)] bg-[var(--dash-red-soft)] p-3 text-sm">
          <div className="font-semibold text-[var(--dash-red)]">Couldn’t approve</div>
