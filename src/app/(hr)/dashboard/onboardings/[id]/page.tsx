...
export default async function OnboardingDetailsPage({
  ...
  const res = await fetchServerPageData<{ onboarding: any }>(
    `/api/v1/admin/onboardings/${id}`,
    { redirectOnSessionRequired: true, homeRedirectPath: "/login" }
  );

  return (
    <OnboardingDetailsClient
      onboardingId={id}
      initialOnboarding={res.data?.onboarding ?? null}
